...
export class AssignmentInventoryService {
  ...
  public getAssignment() {
    ...
    return this.baseService.get<AssignmentDisplayDto[]>(this.getCompleteUrlWithQuery(`${this.endpoint}/GetAssignment`, queryParams));
  }
  ...
  public getLesson() {
    ...
    return this.baseService.get<LessonDisplayDto>(this.getCompleteUrlWithQuery(`${this.endopint2}/GetLesson`, queryParams));
  }
  ...
  public getGrade(){
    ...
    return this.baseService.get<Grade>(this.getCompleteUrlWithQuery(`${this.endpoint}/GetGrade`, queryParams));
  }
  ...
  public deleteAssignment(){
    ...
    return this.baseService.deleteAss<boolean>(`${this.endpoint}/DeleteAssignment?CourseName=${courseName}&LessonTitle=${lessonTitle}`);
  }
  ...
  updateAssignment(lessonTitle: string,courseName: string,assignment: AssignmentDto, file:File): Observable<boolean> {
    const payload = {
      lessonTitle,
  ...
  public addSolution(solution: AssignmentSolutionDto, file: File) {
    ...
    const queryString = `CourseName=${encodeURIComponent(courseName)}&LessonTitle=${encodeURIComponent(lessonTitle)}&StudentEmail=${encodeURIComponent(email)}
    &Solution_title=${encodeURIComponent(solution.solution_title)}&Solution=${encodeURIComponent(solution.solution)}`;

    return this.baseService.addData<boolean>(`${this.endpoint}/Solve?${queryString}`, formData)
        .subscribe({
            next: response => {
  ...
  public addAssignment(assignment: AssignmentDto, file: File){
    ...
    const queryString = `CourseName=${encodeURIComponent(courseName)}&LessonTitle=${encodeURIComponent(lessonTitle)}
    &Assignment_name=${encodeURIComponent(assignment.assignment_name)}
    &Assignment_description=${encodeURIComponent(assignment.assignment_description)}
    &Assignment_preview=${encodeURIComponent(assignment.assignment_preview)}`;

    return this.baseService.addData<boolean>(`${this.endpoint}/AddAssignment?${queryString}`, formData)
        .subscribe({
            next: response => {
