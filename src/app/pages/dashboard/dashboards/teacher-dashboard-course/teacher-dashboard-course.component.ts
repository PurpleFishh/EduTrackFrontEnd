...
export class TeacherDashboardCourseComponent implements OnInit {
  ...
  firstLessonTitle: string = ''; // Add this line
    ...
    this.lessonsService.getAllLessons(course).subscribe({
      next: (lessons: LessonDisplayDto[]) => {
        ...
        if (this.lessons.length > 0) {
          ...
          this.selectedLesson = null; // Reset selected lesson when fetching new lessons
        }
      },
      error: (error) => {
  ...
  fetchAssignments(course: string, lesson: string): void {
    this.assignmentService.getAssignment(course, lesson).subscribe({
      next: (assignments: AssignmentDisplayDto[]) => {
